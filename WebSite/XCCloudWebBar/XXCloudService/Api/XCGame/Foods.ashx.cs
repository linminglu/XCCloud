    ...
    public class Foods : ApiBase
    {
        ...
        public object getfoods(Dictionary<string, object> dicParas)
        {
            try
            {
                ...
                List<XCCloudWebBar.Model.XCGame.t_foods> xcGameFoodsList = null;
                List<XCCloudWebBar.Model.XCCloudRS232.t_foods> rs232FoodsList = null;

                //获取token模式
                MobileTokenModel mobileTokenModel = null;
                XCGameMemberTokenModel memberTokenModel = null;
                if (!XCCloudWebBar.Business.XCGame.MeberAndMobileTokenBusiness.GetTokenData(dicParas, out tokenType, ref mobileTokenModel, ref memberTokenModel, out errMsg))
                {
                ...
                {
                    XCCloudWebBar.BLL.IBLL.XCGame.IFoodsService memberService = BLLContainer.Resolve<XCCloudWebBar.BLL.IBLL.XCGame.IFoodsService>(storeModel.StoreDBName);
                    xcGameFoodsList = memberService.GetModels(p => p.FoodState.Equals("1") && p.FoodType.Equals("0") && p.ForeAuthorize.ToString().Equals("0")).ToList<XCCloudWebBar.Model.XCGame.t_foods>();
                }
                else if (storeModel.StoreType == 1)
                {
                    XCCloudWebBar.BLL.IBLL.XCCloudRS232.IFoodsService memberService = BLLContainer.Resolve<XCCloudWebBar.BLL.IBLL.XCCloudRS232.IFoodsService>();
                    rs232FoodsList = memberService.GetModels(p => p.FoodState == 1).ToList<XCCloudWebBar.Model.XCCloudRS232.t_foods>();
                }
                ...
                else
                {
                    ...
                    {
                        List<FoodsResponseModel> foodsResponseModel = Utils.GetCopyList<FoodsResponseModel, XCCloudWebBar.Model.XCGame.t_foods>(xcGameFoodsList);
                    ...
                    {
                        List<FoodsResponseModel> foodsResponseModel = Utils.GetCopyList<FoodsResponseModel, XCCloudWebBar.Model.XCCloudRS232.t_foods>(rs232FoodsList);
                        SetFoodsResponseModelList(foodsResponseModel);
                        return ResponseModelFactory<List<FoodsResponseModel>>.CreateModel(isSignKeyReturn, foodsResponseModel); 
                    }
                }
        ...
        {
            mobile = string.Empty;
            var memberTokenModel = XCCloudWebBar.Business.XCGame.MemberTokenBusiness.GetMemberTokenModel(memberToken);
        ...
        private bool BuyCoin(XCGameManaDeviceStoreType deviceStoreType, string xcGameDBName, string storeId,int icCardId, int memberLevelId, int foodId, string orderId, string money, int coins, int balance, string paymentype, string deviceId,int deviceIdentityId, out string errMsg)
        {
            ...
            else if (deviceStoreType == XCGameManaDeviceStoreType.Merch)
            {
                ...
                XCCloudWebBar.Model.XCCloudRS232.flw_food_sale flwFood = new Model.XCCloudRS232.flw_food_sale();
