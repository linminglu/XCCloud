...
namespace XCCloudWebBar.Api.XCGameMana
{
    /// <summary>
    /// Token 的摘要说明
    /// </summary>
        ...
        public object getMemberToken(Dictionary<string, object> dicParas)
        {
            ...
            try
            {
                ...
                if (storeModel.StoreType == 0)
                {
                    ...
                    var count = memberService.GetModels(p => p.Mobile.Equals(mobileTokenModel.Mobile, StringComparison.OrdinalIgnoreCase)).Count<XCCloudWebBar.Model.XCGame.t_member>();
                    if (count == 0)
                    {
                ...
                else if (storeModel.StoreType == 1)
                {
                    ...
                    var count = memberService.GetModels(p => p.Mobile.Equals(mobileTokenModel.Mobile, StringComparison.OrdinalIgnoreCase)).Count<XCCloudWebBar.Model.XCCloudRS232.t_member>();
                    if (count == 0)
                    {
