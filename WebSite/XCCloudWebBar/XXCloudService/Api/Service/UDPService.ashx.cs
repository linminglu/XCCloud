    ...
    public class UDPService : ApiBase
    {
        ...
        public object radarRegister(Dictionary<string, object> dicParas)
        {
            try
            {
                ...
                int count = 0;
                UDPClientItemBusiness.ClientItem item = XCCloudWebBar.SocketService.UDP.ClientList.ClientListObj.Where<UDPClientItemBusiness.ClientItem>(p => p.StoreID.Equals(storeId) && p.Segment.Equals(segment)).FirstOrDefault<UDPClientItemBusiness.ClientItem>();
                while (item == null && count < 10)
                {
                    item = XCCloudWebBar.SocketService.UDP.ClientList.ClientListObj.Where<UDPClientItemBusiness.ClientItem>(p => p.StoreID.Equals(storeId) && p.Segment.Equals(segment)).FirstOrDefault<UDPClientItemBusiness.ClientItem>();
        ...
        public object radarHeat(Dictionary<string, object> dicParas)
        {
            ...
            UDPClientItemBusiness.ClientItem item = XCCloudWebBar.SocketService.UDP.ClientList.ClientListObj.Where<UDPClientItemBusiness.ClientItem>(p => p.gID.Equals(token)).FirstOrDefault<UDPClientItemBusiness.ClientItem>();
            if (item == null)
            {
        ...
        public object deviceControl(Dictionary<string, object> dicParas)
        {
            ...
            //判断设备状态是否为启用状态
            XCCloudWebBar.BLL.IBLL.XCGame.IDeviceService ids = BLLContainer.Resolve<XCCloudWebBar.BLL.IBLL.XCGame.IDeviceService>(storeCacheModel.StoreDBName);
            var menlist = ids.GetModels(p => p.MCUID.Equals(mcuid, StringComparison.OrdinalIgnoreCase)).FirstOrDefault<XCCloudWebBar.Model.XCGame.t_device>();
            if (menlist == null)
            {
        ...
        public object getRegisterRadar(Dictionary<string, object> dicParas)
        {
            try
            {
                ...
                StoreBusiness storeBusiness = new StoreBusiness();
                List<UDPClientItemBusiness.ClientItem> clientList = XCCloudWebBar.SocketService.UDP.ClientList.ClientListObj;
