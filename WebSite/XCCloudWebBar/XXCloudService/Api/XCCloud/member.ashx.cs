    ...
    public class Member : ApiBase
    {
        ...
        public object createOperateCardOrder(Dictionary<string, object> dicParas)
        {
            try
            {
                ...
                using (TransactionScope ts = new TransactionScope(TransactionScopeOption.RequiresNew))
                {
                    ...
                    foodSale.ID = RedisCacheHelper.CreateCloudSerialNo(storeId);
        ...
        public object rechangeCard(Dictionary<string, object> dicParas)
        {
            try
            {
                ...
                using (TransactionScope ts = new TransactionScope(TransactionScopeOption.RequiresNew))
                {
                    ...
                    {
                        return ResponseModelFactory.CreateModel(isSignKeyReturn, Return_Code.T, "", Result_Code.F, string.Format("{0}失败", operate));
                    }
        ...
        public object memberTransfer(Dictionary<string, object> dicParas)
        {
            try
            {
                ...
                //【赠送】转入卡在当前门店的余额ID集合
                var toBalanceFreeQuery = from a in Data_Card_Balance_FreeService.N.GetModels(t => t.CardIndex == fromCard.ID && t.MerchID == merchID)
                                         join b in Data_Card_Balance_StoreListService.N.GetModels(t => t.StoreID == storeId) on a.ID equals b.CardBalanceID
                                         select new
                                         {
                                             BalanceId = a.ID,
