            ...
            try
            {
                ...
                if (f.commandData.Length >= 15)
                {
                    ...
                    if (head == null) return;
                    {
                        ...
                        string sql = string.Format("select * from Flw_DeviceData where DeviceID='{0}' and SN='{1}' and BusinessType='{2}'", head.设备编号, 流水号, (int)控制类型);
                        DataTable dt = ac.ExecuteQueryReturnTable(sql);
                        if (dt.Rows.Count > 0)
                        {
                            //重复数据不操作
                            DataRow row = dt.Rows[0];
                            应答数据 = new Ask.Ask液晶卡头进出币指令();
                            应答数据.币余额 = Convert.ToUInt16(row["RemainBalance"]);
                            应答数据.机头地址 = f.commandData[0];
                            应答数据.控制类型 = f.commandData[10];
                            应答数据.控制信号 = Convert.ToByte(row["ACKControlValue"]);
                            应答数据.扣币类型 = f.commandData[11];
                            应答数据.流水号 = 流水号;
                            应答数据.脉冲数 = Convert.ToUInt16(row["ACKPulesCount"]);
                            应答数据.送币数 = Convert.ToUInt16(row["ACKFreeCoin"]);
                        }
                        else
                        {
                            //if (head.是否从雷达获取到状态)
                            {
                                ...
                                if (res == 0)
                                {
                                    ...
                                    Info.CoinInfo.会员卡信息 member = Info.CoinInfo.GetCoinInfo(IC卡号码, 动态密码, head, game);
                                    if (member == null)
                                    {
                                        msg = "未找到有效会员";
                                        return;
                                    ...
                                    应答数据 = new Ask.Ask液晶卡头进出币指令(head, member, game, 扣币类型, 币数, 控制类型, (UInt16)流水号, f, 高速投币标志, 投币目标地址, ref msg);
