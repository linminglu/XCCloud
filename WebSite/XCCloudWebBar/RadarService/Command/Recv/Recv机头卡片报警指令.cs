...
namespace RadarService.Command.Recv
{
    public class Recv机头卡片报警指令
    {
        ...
        public Recv机头卡片报警指令(FrameData f, DateTime RecvDateTime)
        {
            ...
            Info.DeviceInfo.机头信息 机头 = Info.DeviceInfo.GetBufMCUIDDeviceInfo(f.routeAddress, 机头地址);
            if (机头 == null) return;
            ...
            if (res == 0)
            {
                switch (报警类别)
                {
                    ...
                    case 1:
                        ...
                        log.ID = SerialNoHelper.CreateStoreSerialNo(PublicHelper.SystemDefiner.StoreID);
                        log.LockGame = 0;
                        log.LockMember = 0;
