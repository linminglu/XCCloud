    ...
    public class Ask液晶卡头扩展信息下发指令
    {
        ...
        public void SendShowParamet(Info.DeviceInfo.机头信息 head, EndPoint remoteP)
        {
            ...
            if (game != null)
            {
                ...
                Console.WriteLine("游戏机名：" + p.GameName);
                Console.WriteLine("退币类型：" + p.OutName);
                Console.WriteLine("投币1类型：" + p.PushName1);
                Console.WriteLine("投币2类型：" + p.PushName2);
                Console.WriteLine("位置名：" + p.SiteName);
                Console.WriteLine("门店名：" + p.StoreName);
        ...
        string GetQRCode(Info.DeviceInfo.机头信息 head)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    string qr = dt.Rows[0]["BarCode"].ToString();
                    Console.WriteLine("设备序列号：" + head.设备序列号);
                    Console.WriteLine("终端二维码：" + qr);
                    return qr;
                }
            }
            catch (Exception ex)
