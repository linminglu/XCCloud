...
namespace RadarService.Info
{
    public static class CoinInfo
    {
        ...
        static bool 电子退币SQL(DeviceInfo.机头信息 head, UInt16 SN, int Coins, string ICCard, int RepeadCode, CoinType cType, EndPoint p, ref DataMember.IC卡模式进出币应答结构 投币应答, ref string msg)
        {
            ...
            if (head.退币保护.退币保护启用标志 && head.退币保护.退币信号超时退币个数 - 1 <= Coins && head.退币保护.退币信号超时退币个数 + 1 >= Coins)
            {
                ...
                if (DeviceInfo.TBProtect(head))
                {
                    ...
                    //锁定会员卡
                    object o = new Data_Member_Card();
                    DataModel model = new DataModel();
                    model.CovertToDataModel("select * from Data_Member_Card where ID='" + member.会员卡索引 + "'", ref o);
                    Data_Member_Card c = o as Data_Member_Card;
                    c.IsLock = 1;
                    c.Verifiction = model.Verifiction(c, PublicHelper.SystemDefiner.AppSecret);
                    model.Update(c, "where ID='" + member.会员卡索引 + "'");
                    //创建报警信息
                    Log_GameAlarm alerm = new Log_GameAlarm();
                    ...
                    alerm.Verifiction = model.Verifiction(alerm, PublicHelper.SystemDefiner.AppSecret);
                    model.Add(alerm);
                    return false;
                }
            }
            else
            {
                ...
                Flw_DeviceData dd = new Flw_DeviceData();
        ...
        public static DataMember.液晶卡头进出币应答结构 液晶卡头进出币(Info.DeviceInfo.机头信息 head, 会员卡信息 member, GameInfo.游戏机信息 game, int Coins, byte UseType, UInt16 SN, CoinType cType, ref string msg, bool isPush, string PushAddr, out int FreeCoin, out int PushIndex)
        {
            PushIndex = 0;
            FreeCoin = 0;
            ...
            {
                case CoinType.电子投币:
                    if (!液晶卡头投币(head, Coins, UseType, cType, member, game, SN, ref 应答结构, ref msg, ref SkipWriteFlw, out FreeCoin, out PushIndex))
                    {
                        return 应答结构;
                    }
                    break;
                case CoinType.电子退币:
                    if (!液晶卡头退币(head, Coins, cType, member, game, SN, ref 应答结构, ref msg))
                    {
                        return 应答结构;
                    }
                    break;
        ...
        static void 会员余额扣除(string cardIndex, int balanceIndex, decimal balance)
        {
            ...
            f.Verifiction = model.Verifiction(f, PublicHelper.SystemDefiner.AppSecret);
            model.Update(f, "where ID='" + f.ID + "'");
        }
        static void 会员余额退币(string cardIndex, int balanceIndex, decimal balance)
        {
            DataModel model = new DataModel();
            object o = new Data_Card_Balance();
            model.CovertToDataModel("select * from Data_Card_Balance where CardIndex='" + cardIndex + "' and BalanceIndex='" + balanceIndex + "'", ref o);
        ...
        public static bool 液晶卡头退币(Info.DeviceInfo.机头信息 head, int Coins, CoinType cType, 会员卡信息 member, GameInfo.游戏机信息 game, UInt16 SN, ref DataMember.液晶卡头进出币应答结构 投币应答, ref string msg)
        {
            投币应答.超出当日机头最大净退币上线 = false;
            投币应答.机头能上分 = member.允许投币;
            投币应答.机头能打票 = member.允许退币;
            ...
            int Multiple = Coins / game.退币参数.退币时游戏机出币基数;
            int TBCount = game.退币参数.退币时余额增加基数 * Multiple;
            投币应答.币余额 = member.退币余额 + TBCount;
            会员余额退币(member.会员卡索引, game.退币参数.退币余额类别, TBCount);

            return true;
        }
        public static bool 液晶卡头投币(Info.DeviceInfo.机头信息 head, int Coins, byte UseType, CoinType cType, 会员卡信息 member, GameInfo.游戏机信息 game, UInt16 SN, ref DataMember.液晶卡头进出币应答结构 投币应答, ref string msg, ref bool SkipWriteFlw, out int FreeCoin, out int PushIndex)
        {
            FreeCoin = 0;
            msg = "";
            SkipWriteFlw = true;
            ...
            if ((member.第一余额 >= Coins && UseType == 1) || (member.第二余额 >= Coins && UseType == 2))
            {
                ...
                会员余额扣除(member.会员卡索引, PushIndex, Coins);
        ...
        public static bool 门票消耗(Info.DeviceInfo.机头信息 head, 会员卡信息 member, GameInfo.游戏机信息 game, int Coins, byte UseType, ref DataMember.液晶卡头进出币应答结构 投币应答)
        {
            if (game.游戏机消费类别 == 2)
            {
                if (head.验票进出方向 == 0)
                {
                    ...
                    {
                        Info.ProjectTicketInfo.门票信息 ticket = SerialNoHelper.StringGet<Info.ProjectTicketInfo.门票信息>("ticketin_" + head.设备序列号);
        ...
        {
            DataModel model = new DataModel();
            object o = new Flw_Project_TicketUse();
            if (model.CovertToDataModel("select * from Flw_Project_TicketUse where ID='" + flwID + "'", ref o))
            {
                Flw_Project_TicketUse ticket = o as Flw_Project_TicketUse;
                ticket.OutMinuteTotal += (int)(DateTime.Now - (DateTime)ticket.OutTime).TotalMinutes;
                ticket.OutTime = null;
                ticket.Verifiction = model.Verifiction(ticket, PublicHelper.SystemDefiner.AppSecret);
        ...
        static bool 游乐项目正常入闸(Info.DeviceInfo.机头信息 head, Info.CoinInfo.会员卡信息 member, GameInfo.游戏机信息 game, string BindID, ref DataMember.液晶卡头进出币应答结构 投币应答)
        {
            ...
            if (ticket != null)
            {
                ...
                if (ticket.RemainCount > 0)
                {
                    ...
                    t.OutMinuteTotal = 0;
                    t.OutTime = null;
                    t.Verifiction = model.Verifiction(t, PublicHelper.SystemDefiner.AppSecret);
                    model.Add(t);
                    //新增闸机进出记录
                    Flw_Project_TicketDeviceLog l = new Flw_Project_TicketDeviceLog();
                    ...
                    ticket.票.Verifiction = model.Verifiction(ticket.票, PublicHelper.SystemDefiner.AppSecret);
                    model.Update(ticket.票, "where ID='" + ticket.票.ID + "'");
                    //获取指定门票绑定信息
                    object o = new Flw_ProjectTicket_Bind();
                    if (model.CovertToDataModel("select * from Flw_ProjectTicket_Bind where ID='" + BindID + "'", ref o))
                    {
                        Flw_ProjectTicket_Bind b = o as Flw_ProjectTicket_Bind;
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (game != null)
                    {
                        ...
                        if (game.退币参数.退币余额类别 > 0)
                        {
                            card.退币余额 = Convert.ToInt32(GetBalanceCount(card.会员卡索引, game.退币参数.退币余额类别));
                        }
                    }
                    else
                    {
