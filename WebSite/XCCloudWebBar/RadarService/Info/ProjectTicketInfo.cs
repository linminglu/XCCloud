...
namespace RadarService.Info
{
    ...
    public class ProjectTicketInfo
    {
        public class 门票信息
        {
            ...
            public Flw_Project_TicketInfo 票 { get; set; }
        }
        ...
        static bool 期限票判断使用频率规则(DataRow row)
        {
            ...
            if (share && ptype == 0)
                ...
                //次数不共享，按小时间隔
                sql = "select COUNT(u.ID) as num from Flw_Project_TicketUse u,Data_Project_BindDevice b where u.DeviceID=b.DeviceID and u.ProjectTicketCode='" + barcode + "' and b.ProjectID='" + projectID + "' and DATEADD(mi," + pvalue + ",u.InTime)>=GETDATE()";
            else
                //次数不共享，按天间隔
                sql = "select COUNT(u.ID) as num from Flw_Project_TicketUse u,Data_Project_BindDevice b where u.DeviceID=b.DeviceID and u.ProjectTicketCode='" + barcode + "' and b.ProjectID='" + projectID + "' and DATEADD(d," + pvalue + ",u.InTime)>=GETDATE()";
            DataAccess ac = new DataAccess();
            DataTable dt = ac.ExecuteQueryReturnTable(sql);
