...
namespace RadarService.Coding
{
    public static class ConvertData
    {
        ...
        public static string GetTicketDate(byte[] b)
        {
            return (2000 + b[0]).ToString() + "-" + b[1].ToString() + "-" + b[2].ToString();
        }
        ...
        public static byte[] GetHeadParamet(Info.GameInfo.显示参数 paramet)
        {
            List<byte> buf = new List<byte>();
            byte cmdType = 0;
            Type t = paramet.GetType();
            foreach (PropertyInfo pi in t.GetProperties())
            {
                string v = pi.GetValue(paramet, null).ToString();
                if (v == null)
                    v = "";
                buf.Add(cmdType++);
                if (v.Length > 0)
                {
                    byte[] code = Encoding.GetEncoding("gb2312").GetBytes(v);
                    buf.Add((byte)code.Length);
                    buf.AddRange(Encoding.GetEncoding("gb2312").GetBytes(v));
                }
                else
                    buf.Add(0x00);
