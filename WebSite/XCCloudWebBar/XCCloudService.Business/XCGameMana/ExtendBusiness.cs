...
namespace XCCloudWebBar.Business.XCGameMana
{
    public class ExtendBusiness
    {
        ...
        public static bool checkXCGameManaDeviceInfo(string deviceToken, out XCGameManaDeviceStoreType deviceStoreType, out string storeId, out string deviceId)
        {
            ...
            var deviceModel = deviceService.GetModels(p => p.TerminalNo.Equals(deviceToken, StringComparison.OrdinalIgnoreCase)).FirstOrDefault<XCCloudWebBar.Model.XCGameManager.t_device>();
            if (deviceModel == null)
            {
                return false;
            }
        ...
        public static bool checkStoreDeviceInfo(XCGameManaDeviceStoreType deviceStoreType, string storeId, string deviceId, out string segment, out string mcuId, out string xcGameDBName, out int deviceIdentityId,out string storePassword,out string storeName,out string errMsg)
        {
            ...
            if (deviceStoreType == XCGameManaDeviceStoreType.Store)
            {
                ...
                XCCloudWebBar.BLL.IBLL.XCGame.IDeviceService xcGameDeviceService = BLLContainer.Resolve<XCCloudWebBar.BLL.IBLL.XCGame.IDeviceService>(xcGameDBName);
                var xcGameDeviceModel = xcGameDeviceService.GetModels(p => p.MCUID.Equals(deviceId, StringComparison.OrdinalIgnoreCase)).FirstOrDefault<XCCloudWebBar.Model.XCGame.t_device>();
            ...
            {
                XCCloudWebBar.BLL.IBLL.XCCloudRS232.IBase_DeviceInfoService deviceService = BLLContainer.Resolve<XCCloudWebBar.BLL.IBLL.XCCloudRS232.IBase_DeviceInfoService>();
                var deviceModel = deviceService.GetModels(p => p.SN.Equals(deviceId, StringComparison.OrdinalIgnoreCase)).FirstOrDefault<XCCloudWebBar.Model.XCCloudRS232.Base_DeviceInfo>();
                if (deviceModel == null)
                {
                    errMsg = "设备不存在";
                    return false;
        ...
        public static bool checkStoreGameDeviceInfo(XCGameManaDeviceStoreType deviceStoreType, string storeId, string deviceId, out string segment, out string mcuId,out string headId,out string gameId,out string xcGameDBName,out string storePassword, out string storeName, out string errMsg)
        {
            ...
            if (deviceStoreType == XCGameManaDeviceStoreType.Store)
            {
                ...
                var headModel = headService.GetModels(p => p.MCUID.Equals(deviceId) && p.State.Equals("1")).FirstOrDefault<XCCloudWebBar.Model.XCGame.t_head>();
                if (headModel == null)
                {
                    errMsg = "设备门店头部信息不存在";
                    return false;
                ...
                var gameModel = gameService.GetModels(p => p.GameID.Equals(headModel.GameID) && p.State.Equals("1")).FirstOrDefault<XCCloudWebBar.Model.XCGame.t_game>();
                if (gameModel == null)
                {
                    errMsg = "设备门店头部信息不存在";
                    return false;
        ...
        public static bool GetMemberInfo(XCGameManaDeviceStoreType deviceStoreType, string mobile, string xcGameDBName, out int balance, out int icCardId, out int memberLevelId, out string errMsg)
        {
            ...
            if (deviceStoreType == XCGameManaDeviceStoreType.Store)
            {
                ...
                if (XCCloudWebBar.Business.XCGame.MemberBusiness.IsEffectiveStore(mobile, xcGameDBName, ref memberModel, out errMsg))
                {
            ...
            {
                XCCloudWebBar.Model.XCCloudRS232.t_member memberModel = null;
                if (XCCloudWebBar.Business.XCCloud.MemberBusiness.IsEffectiveStore(mobile, ref memberModel, out errMsg))
                {
                    balance = Convert.ToInt32(memberModel.Balance);
