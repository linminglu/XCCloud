    ...
    public class Member : ApiBase
    {
        ...
        public object QueryMemberLevel(Dictionary<string, object> dicParas)
        {
            try
            {
                ...
                var query = data_MemberLevelService.GetModels(p => p.MerchID.Equals(merchId, StringComparison.OrdinalIgnoreCase));
                if (!string.IsNullOrEmpty(memberLevelID))
                {
            ...
            try
            {
                ...
                using (TransactionScope ts = new TransactionScope(TransactionScopeOption.RequiresNew))
                {
                    ...
                    foreach (var item in MemberFreeList)
                    {
                        ...
                        else if (item.FreeType == 2 && level.ID.ToString() == item.FreeId)
                        {
                        ...
                        else if (item.FreeType == 3 && level.ID.ToString() == item.FreeId)
                        {
                            //当前赠币详情
                            var currFreeDetail = item.FreeDetails.FirstOrDefault();
                            //添加输赢赠币记录
