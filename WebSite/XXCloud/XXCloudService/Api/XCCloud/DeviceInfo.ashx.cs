    ...
    public class DeviceInfo : ApiBase
    {
        ...
        public object QueryDeviceInfo(Dictionary<string, object> dicParas)
        {
            try
            {
                ...
                var base_DeviceInfo = (from a in query
                                       join b in
                                           (
                                               from b in Data_GameInfoService.N.GetModels()
                                       ...
                                       select new Base_DeviceInfoList
                                       {
                                           ...
                                           AreaName = b != null ? b.AreaName : string.Empty,
                                           SiteName = a.SiteName,
                                           segment = a.segment,
                                           Address = a.Address,
                                           DeviceRunning = string.Empty,
                                           DeviceStatus = a.DeviceStatus,
                                           DeviceStatusStr = a.DeviceStatus != null ? (a.DeviceStatus == 0 ? "停用" : a.DeviceStatus == 1 ? "正常" : a.DeviceStatus == 2 ? "锁定" : string.Empty) : string.Empty
                                       }).ToList();
        ...
        public object BindDeviceInfo(Dictionary<string, object> dicParas)
        {
            try
            {
                ...
                else if (iState == 2)
                {
                    ...
                    if (!Base_DeviceInfoService.I.Update(data_DeviceInfoNew, true, merchId, merchSecret))
                    {
                        errMsg = "机台替换失败";
                        return ResponseModelFactory.CreateFailModel(isSignKeyReturn, errMsg);
                    }
                ...
                if (!Base_DeviceInfoService.I.Update(data_DeviceInfo, true, merchId, merchSecret))
                {
                    errMsg = "操作失败";
                    return ResponseModelFactory.CreateFailModel(isSignKeyReturn, errMsg);
                }
        ...
        public object SaveGameWatchFromProgram(Dictionary<string, object> dicParas)
        {
            try
            {
                ...
                if (!flw_Game_WatchService.Add(flw_Game_Watch, true, merchId, merchSecret))
                {
                    errMsg = "保存码表抄账信息失败";
                    return ResponseModelFactory.CreateFailModel(isSignKeyReturn, errMsg);
                }
                ...
                using (TransactionScope ts = new TransactionScope())
                {
                    try
                    {
                        ...
                        if (base_DeviceInfo_Ext.ID == 0)
                        {
                            ...
                            if (!base_DeviceInfo_ExtService.Add(base_DeviceInfo_Ext, true, merchId, merchSecret))
                            {
                                errMsg = "售币机参数设置失败";
                                return ResponseModelFactory.CreateFailModel(isSignKeyReturn, errMsg);
                            }
                        ...
                        {
                            if (!base_DeviceInfo_ExtService.Update(base_DeviceInfo_Ext, true, merchId, merchSecret))
                            {
                                errMsg = "售币机参数设置失败";
                                return ResponseModelFactory.CreateFailModel(isSignKeyReturn, errMsg);
                            }
            ...
            try
            {
                ...
                using (TransactionScope ts = new TransactionScope())
                {
                    try
                    {
                        ...
                        base_DeviceInfoService.UpdateModel(base_DeviceInfo, true, merchId, merchSecret);
                        ...
                        foreach (var bindModel in bindDevices)
                        {
                            ...
                            base_DeviceInfoService.UpdateModel(bindModel, true, merchId, merchSecret);
                        }
