                ...
                try
                {
                    ...
                    var linq = from a in dbContext.Set<Base_UserInfo>().Where(p => p.UserID == iUserId)
                               ...
                               select new
                               {
                                   ...
                                   SwitchMerch = a.SwitchMerch,
                                   SwitchStore = a.SwitchStore,
                                   SwitchWorkstation = a.SwitchWorkstation
                               };
        ...
        #region "获取用户注册短信验证码"
            ...
            public object getRegisterSMSCode(Dictionary<string, object> dicParas)
            {
                try
                {
                    ...
                    if (!Base_StoreInfoService.I.Any(a => a.StoreID.Equals(storeId, StringComparison.OrdinalIgnoreCase)))
                    {
                        return ResponseModelFactory.CreateModel(isSignKeyReturn, Return_Code.T, "", Result_Code.F, "该门店信息不存在");
                    }                    
