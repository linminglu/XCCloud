...
namespace XCCloudService.Redis
{
    ...
    public class RedisStackExchangeHelper
    {
        ...
        /// <param name="key"></param>
        /// <returns></returns>
        public RedisValue[] SortedSetRangeByRank<T>(string key, long start, long end, StackExchange.Redis.Order order)
        {
            key = AddSysCustomKey(key);
            return Do(redis =>
            {
                var values = redis.SortedSetRangeByRank(key, start, end, order);
                return values;
            });
        }
