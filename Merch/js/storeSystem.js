...
xcActionSystem.prototype = {
    ...
    TimeSet: function (table, foodLevels, id, mLIDs, week) {
        ...
        var Week = ';'
        for (var i in mLIDs) {
            mlIDS1.push(mLIDs[i].value);
            mlIDS2.push(mLIDs[i].title);
        }
        if (timeType == 1) {
            for (var i in week) {
                week1.push(week[i].value);
                week2.push(week[i].title);
                Week = week1.join('|');
                WeekStrs = week2.join('|');
                typeChecks = 3;
            }
        ...
        var memberLevelIDs = mlIDS1.join('|');
        var memberLevels = mlIDS2.join('|');
        ...
        if (memberLevelIDs != "" && memberLevelIDs != null && memberLevelIDs != undefined) {
            foodLevels.push({
                ...
                'Time': time,
                'coins': coinss,
                'allowCoin': allowCoins,
                'allowPoint': allowPoints,
                'points': points,
                'allowLottery': allowLotterys,
                'lottery': lottery,
                'ClientPrice': ClientPrice,
                'VIPPrice': VipPrice,
                'day_sale_count': dsc,
                'member_day_sale_count': mdsc
            });
            ...
            success: function (data) {
                ...
                if (data.result_code == 1) {
                    ...
                    for (let i in arr) {
                        if (selected != undefined) {
                            if (arr[i].ID == selected) {
                                $('#' + id).append('<option value="' + arr[i].ID + '" selected   title="' + arr[i].EndDate + '">' + arr[i].CouponName + '</option>')
                            } else {
                                $('#' + id).append('<option value="' + arr[i].ID + '" title="' + arr[i].EndDate + '">' + arr[i].CouponName + '</option>')
                            }
                        } else {
                            $('#' + id).append('<option value="' + arr[i].ID + '"  title="' + arr[i].EndDate + '">' + arr[i].CouponName + '</option>')
                        }
    ...
    //1.获取未绑定设备字典
    getUnBindDeviceDic: function (token, layer, form, id, selected) {
        let _obj = {'userToken': token, 'signkey': '1f626576304bf5d95b72ece2222e42c3'};
        let parseJson = JSON.stringify(_obj);
        $.ajax({
            type: 'post',
    ...
    //供应商自典
    getGoodSupplierDic: function (token, layer, form, id, selected) {
        let _obj = {'userToken': token, 'signkey': '1f626576304bf5d95b72ece2222e42c3'};
        let parseJson = JSON.stringify(_obj);
        $.ajax({
            type: 'post',
                        ...
                        if (selected != undefined) {
                            for (let i = 1; i < arr.length; i++) {
                                if (arr[i].children.length > 0) {
                                    $('#' + id).append("<optgroup  label='" + arr[i].name + "'>");
                                    for (let j in arr[i].children) {
                                        if (arr[i].children[j].id == selected) {
                                            $('#' + id).append('<option value="' + arr[i].children[j].id + '" selected>' + arr[i].children[j].name + '</option>')
                                        } else {
                                            $('#' + id).append('<option value="' + arr[i].children[j].id + '">' + arr[i].children[j].name + '</option>')
                                        }
